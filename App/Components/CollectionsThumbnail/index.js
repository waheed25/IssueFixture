import React from 'react'
import { Platform, TouchableHighlight } from 'react-native'
import styled from 'styled-components/native'
import { Card } from 'native-base'
import FastImage from 'react-native-fast-image'
import { Grid, Row, Col } from 'react-native-easy-grid'
import DeviceSizes from '../../Styles/DeviceSizes'
import Images from '../../Images/rootImages'
const url = 'data:image/jpeg;base64,'

const underlayColor = 'transparent'
export default class CollectionsThumbnail extends React.Component {
  shouldComponentUpdate (nextProps, nextState, nextContext): boolean {
    return false
  }

  renderThreeImages = ({ images, creationsCount, collectionName, onPress, collectionInfo }) => {
    return (
      <TouchableHighlight onPress={() => onPress(collectionInfo)} underlayColor={underlayColor}>
        <CardView>
          <Grid style={{ flex: 0 }}>
            <Col size={60} style={{ flex: 1 }}>
              <LeftImageView left={1} right={0}>
                <ImageView source={{ uri: images[0] || url }} />
              </LeftImageView>
            </Col>
            <Col size={40} style={{ flex: 1 }}>
              <WrapperView left={1} right={0}>
                <Row style={{
                  borderTopRightRadius: 10,
                  overflow: 'hidden',
                  borderBottomWidth: 2,
                  borderBottomColor: 'white'
                }}>
                  <ImageView
                    style={{ flex: 1 }}
                    source={{ uri: images[1] || url }}
                />
                </Row>
                <Row>
                  <ImageView
                    style={{ flex: 1 }}
                    source={{ uri: images[2] || url }}
                />
                </Row>
              </WrapperView>
            </Col>
          </Grid>
          <CollectionTitle ellipsizeMode='tail' numberOfLines={2}>{collectionName}</CollectionTitle>
          <BottomView>
            <CollectionTitle style={{ marginBottom: 10 }} ellipsizeMode='tail'
              numberOfLines={2}>{creationsCount} Creations</CollectionTitle>
          </BottomView>
        </CardView>
      </TouchableHighlight>
    )
  };
  renderTwoImages = ({ images, creationsCount, collectionName, onPress, collectionInfo }) => {
    return (
      <TouchableHighlight onPress={() => onPress(collectionInfo)} underlayColor={underlayColor}>
        <CardView>
          <Grid style={{ flex: 0 }}>
            <Col size={50} style={{ flex: 1 }}>
              <LeftImageView>
                <ImageView source={{ uri: images[0] || url }} />
              </LeftImageView>
            </Col>
            <Col size={50} style={{ flex: 1 }}>
              <WrapperView left={1} right={0}>
                <ImageView source={{ uri: images[1] || url }} />
              </WrapperView>
            </Col>
          </Grid>
          <CollectionTitle ellipsizeMode='tail' numberOfLines={2}>{collectionName}</CollectionTitle>
          <BottomView>
            <CollectionTitle style={{ marginBottom: 10 }} ellipsizeMode='tail' numberOfLines={2}>
              {creationsCount} Creations</CollectionTitle>
          </BottomView>
        </CardView>
      </TouchableHighlight>
    )
  }
  renderOneImage=({ images, creationsCount, collectionName, onPress, collectionInfo }) => {
    return (
      <TouchableHighlight onPress={() => onPress(collectionInfo)} underlayColor={underlayColor}>
        <CardView>
          <WrapperView>
            <FastImage style={{ height: '100%', flex: 1 }} resizeMode='cover' source={{ uri: images[0] || '' }} />
          </WrapperView>
          <CollectionTitle ellipsizeMode='tail' numberOfLines={2}>{collectionName}</CollectionTitle>
          <BottomView>
            <CollectionTitle style={{ marginBottom: '2%' }} ellipsizeMode='tail' numberOfLines={2}>{creationsCount} Creations</CollectionTitle>
          </BottomView>
        </CardView>
      </TouchableHighlight>
    )
  }
  renderDefaultImage=({data, collectionInfo}) => {
    const thumbnailOne = JSON.parse(data[0].attributes.mediaMetadata)
    return (
      <CardView>
        <WrapperView>
          <FastImage style={{ height: '100%', flex: 1 }} resizeMode='cover' source={{ uri: url }} />
        </WrapperView>
        <CollectionTitle ellipsizeMode='tail' numberOfLines={2}>{collectionInfo.attributes.title}</CollectionTitle>
        <BottomView>
          <CollectionTitle style={{ marginBottom: '2%' }} ellipsizeMode='tail' numberOfLines={2}>{collectionInfo.attributes.creationsCount} Creations</CollectionTitle>
        </BottomView>
      </CardView>
    )
  }
  renderEmptyCollection =({ collectionName, creationsCount }) => {
    return (<CardView>
      <WrapperView>
        <FastImage style={{ height: '100%', flex: 1 }} resizeMode='cover' source={Images.placeHolder} />
      </WrapperView>
      <CollectionTitle ellipsizeMode='tail' numberOfLines={2}>{collectionName}</CollectionTitle>
      <BottomView>
        <CollectionTitle style={{ marginBottom: '2%' }} ellipsizeMode='tail' numberOfLines={2}>{creationsCount} Creations</CollectionTitle>
      </BottomView>
    </CardView>
    )
  }

  render () {
    const { images, creationsCount, collectionName, onPress, collectionInfo } = this.props
    console.log('collections thumbnail data', this.props)
    return (

      <ContainerView>

        {
          images.length === 0 && this.renderEmptyCollection({ collectionName, creationsCount, onPress, collectionInfo })
        }
        {
          images.length === 1 && this.renderOneImage({ images, collectionName, creationsCount, onPress, collectionInfo })
        }
        {
          images.length === 2 && this.renderTwoImages({ images, collectionName, creationsCount, onPress, collectionInfo })
        }
        {
          images.length === 3 && this.renderThreeImages({ images, collectionName, creationsCount, onPress, collectionInfo })
        }
      </ContainerView>

    )
  }
}
const ContainerView = styled.View`
justify-content: center;
align-items: center;
`
const CardView = styled(Card)`
height: ${DeviceSizes.DEVICE_HEIGHT * 0.40};
borderTopLeftRadius: 10;
width: ${DeviceSizes.DEVICE_WIDTH * 0.93};
border-radius: 10;
`
const ImageView = styled(FastImage)`
height: 100%;
`
const WrapperView = styled.View`
overflow: hidden;
height: ${Platform.OS === 'android' ? DeviceSizes.DEVICE_HEIGHT * 0.28 : DeviceSizes.DEVICE_HEIGHT * 0.30}
borderTopLeftRadius: ${props => props.left ? props.left : 10};
borderTopRightRadius: ${props => props.right ? props.right : 10};
borderRightWidth: ${props => props.border ? props.border : 0};
borderRightColor: white
`
const LeftImageView = styled(Col)`
borderTopLeftRadius: 10;
overflow: hidden;
borderRightWidth: 2;
borderRightColor: white
`
const CollectionTitle = styled.Text`
font-weight: 600;
width: ${DeviceSizes.DEVICE_WIDTH * 0.7};
margin-left: 10;
margin-top: 10;
`
const BottomView = styled.View`
position: absolute;
bottom: 0;
`
